...
export default function Page() {
    ...
    function getStoryIdFromUrl(url: string): string | null {
        try {
            const parsed = new URL(url);
            const parts = parsed.pathname.split("/").filter(Boolean);
            // contoh: ["stories", "plndistribusibali", "3737276771969679069"]
            return parts[2] && /^\d+$/.test(parts[2]) ? parts[2] : null;
        } catch {
            return null;
        }
    }
    ...
    const submit = async () => {
        ...
        try {
            ...
            const storyId = getStoryIdFromUrl(url)

            const stories = dataIG.result
            const story = stories.filter((s: StoryItem) => s.pk === storyId)
            ...
            const prompt = `Baca dan ekstrak informasi pemeliharaan jaringan listrik dari gambar ini.  Sajikan output hanya dalam format JSON dengan key tanggal_pemeliharaan, unit_pelaksana dan lokasi_pemeliharaan. untuk key lokasi_pemeliharaan berupa object array dengan key ulp, waktu dan lokasi. Format waktu harus dd MMMM YYYY`
